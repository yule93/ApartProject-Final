...
var mapOption = {
  center: new kakao.maps.LatLng(parsedCoords.latitude, parsedCoords.longitude), // 지도의 중심좌표
  level: 3, // 지도의 확대 레벨
};
...
function displayMarker(place, delay) {
  var marker = new kakao.maps.Marker({
    position: new kakao.maps.LatLng(place.y, place.x),
  });

  var points = [center, marker.getPosition()];
...
// 치킨 검색하기
function displayChickenMarkers(places) {
...
function displayMarkers(places) {
  ...
  removeMarker();
  for (var i = 0; i < places.length; i++) {
    var placePosition = new kakao.maps.LatLng(places[i].lat, places[i].lan);
    var marker = addMarker(placePosition, i);
    var itemEl = getListItem(i, places[i]); // 검색 결과 항목 Element를 생성합니다
